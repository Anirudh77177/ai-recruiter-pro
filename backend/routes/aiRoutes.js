...
router.post('/generate-interview', async (req, res) => {
    try {
        ...
        // YE LINE ADD KARNI HAI TAASI TERMINAL PE ERROR DIKHE
        console.error("🔥 BACKEND ERROR IN GENERATE Q&A:", error); 
        res.status(500).json({ error: error.message });
    }
});
...
router.put('/candidates/:id/schedule', async (req, res) => {
    try {
        ...
        if (!candidate) return res.status(404).json({ message: "Candidate not found" });
        res.status(200).json({ data: candidate, message: "Interview Scheduled!" });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/parse-resume/:id', async (req, res) => {
    try {
        ...
        if (candidate.parsedResume && Object.keys(candidate.parsedResume).length > 0) {
            console.log("✅ Data found in DB, skipping API call.");
